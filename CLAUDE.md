...
## Stack
- React + Vite, arquivo único: `src/App.jsx` (~4800 linhas)
...
- `businessWidget`: widget flutuante de Business Impact — `{visible: boolean, x: number, y: number, w: number, h: number}`
...
- `startBwDrag(e, type, dir)`: inicia drag (`type='move'`) ou resize (`type='resize'`, `dir` em `n|s|e|w|ne|nw|se|sw`) do Business Impact widget via `window.addEventListener`
...
- `SimIndicators`: card executivo de Business Impact — **não está mais no painel lateral**; mantido como componente mas não renderizado na sidebar. O conteúdo equivalente foi integrado ao widget flutuante.
...
### Padrão de refs
Toda variável de estado tem um ref espelho (`vpR`, `shapesR`, `axisModalR`, `businessWidgetR`, etc.) para uso em event listeners sem closure stale. `bwDragR` armazena o estado ativo de drag/resize do Business Widget.
...
- Nó SVG no canvas; exibe Taxa de Aprovação, Inad. Real e Inad. Inferida via `foreignObject`
- Sidebar direita **não** exibe mais o card Business Impact inline (ver widget abaixo)

## Widget Flutuante — Business Impact (`businessWidget`)

### Arquitetura
- Elemento HTML `position: absolute` dentro do container canvas (`flex:1, position:relative`) — **não** é um nó SVG
- Estado: `{ visible: boolean, x: number, y: number, w: number, h: number }`
- Ref espelho: `businessWidgetR`; drag ativo: `bwDragR`

### Ativação
Painel lateral direito → seção "Simulação" → checkbox **"Exibir no board"** ou botão **"Abrir Widget"**.

### Drag & Resize
- `startBwDrag(e, 'move')` no header → move o widget livremente
- `startBwDrag(e, 'resize', dir)` nos 8 handles → redimensiona em qualquer direção
- Limites: `minWidth: 320px`, `minHeight: 220px`, `maxWidth: 1400px`, `maxHeight: 1200px`
- Implementação usa `window.addEventListener('mousemove'/'mouseup')` criado/destruído dinamicamente — sem interferir no SVG

### Escala dinâmica de conteúdo
```js
const sf = Math.max(0.55, Math.min(2.5, bwW / 420))
const s  = (v) => v * sf   // aplica a fontSize, padding, gap, borderRadius, etc.
```

### Breakpoints
| Tamanho | Condição | Comportamento |
|---------|----------|---------------|
| Small   | `w < 380` | Oculta grid de métricas, balance bar e badges; exibe só hero KPI |
| Medium  | `380 ≤ w < 620` | Grid 2×2 completo + balance bar estratégico |
| Large   | `w ≥ 620` | Adiciona painel "Análise de Portfolio" (3 colunas: Total/Aprovados/Reprovados) |

### Conteúdo
1. **Header** (drag handle): título ⬡ Business Impact · total de registros · botão ✕
2. **Badges contextuais**: Δ Aprovação, nível de risco (small: ocultos)
3. **Hero KPI**: taxa de aprovação ou variação incremental em fonte `42 * sf`
4. **Barra de progresso** da aprovação com marcador de baseline incremental
5. **Grid 2×2**: Aprovação · Inad. Real · Inad. Inferida · Vol. Aprovado (com deltas)
6. **Balance bar**: Risco ◄ ─── ● ─── ► Crescimento com indicador animado
7. **Efeito da Mudança** (quando `incrementalResult` presente): Novos Aprovados / Novos Reprovados
8. **Análise de Portfolio** (large only): 3 cards com volume total/aprovado/reprovado
...
- Persistência de `businessWidget` (posição/tamanho) no export/import de fluxo
...
`claude/floating-business-impact-widget-7Su4A`
