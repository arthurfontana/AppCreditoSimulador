...
| `tests/policyJourney.test.js` | Etapas + Prontidão (Jornada EP1) — `detectJourneyStages` (6 detectores: knock-out ≤2, segmentação, corte de risco, calibração, override manual respeitado, E6=checklist) e `computeReadiness` (critérios ≡ lint/funil/pendências/`diffPolicyIR`/simplify/flags do BaseProfile, número a número; `na` por config; determinismo) |
