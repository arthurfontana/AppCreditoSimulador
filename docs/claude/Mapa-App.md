...
|---|--------|-------------|------------------------------------------|---------------------|
| 1 | `Constantes e Helpers Globais` | 17 | `BuildBadge`, `ComputeEngineBadge`, `ComputeCeilingNotice`, `ComputeFallbackNotice`, `ComputeJobProgress`, `SidecarTestPanel`, `PerfDebugPanel`, `SW`/`SH`, `COLORS`, `DELIMITERS`, `parseCSV`, `detectDelimiter`/`detectDecimalSep`, `suggestVarType`, `suggestMetricColumns`, `buildFlowGraph`, `validateFlow`, `exportDiagnosticCSV`, `computeAsIsCells`, `SimIndicators`, `computeWidgetMetric`, `pivotWidget`, `applyAnalyticsFilters`/`applyFiltersToDataset`, `autoBuckets`, `applyGroupingsToDataset` | Badges/telemetria do Motor Python, parsing de CSV bruto, grafo de fluxo/validação, export diagnóstico, cálculo AS IS e indicadores de simulação, e as funções puras do Analytics Workspace (filtros, agrupamentos, pivot) usadas por `AnalysisTab` |
...
| 4 | `PolicyIR` | 4030 | `buildPolicyIR`, `applyPolicyPatch`, `extractPolicyRequiredVars`, `applyPolicyVarMapping`, `diffPolicyIR`, `hashPolicyIR`, `describeLensRule(s)`/`describeCondition`/`describeFlowNode`/`describePath`, `renderDocMarkdown`, `renderDocHTML` | JSON canônico da política (Copiloto Sessão 0): serialização do canvas para IR, aplicação/patch de IR de volta ao canvas, diff entre versões, e os renderizadores de Documentação Automática (Markdown/HTML) que consomem o IR |
