...
| `COMPUTE_CLUSTER_SEGMENTS` | `{params, shapes?, conns?, scope?}` | **Execução Híbrida H8 + Épico FR (DEC-FR-001, FR1)** — Clusterização de Segmentos (baseline browser, Classe B dentro dos tetos). `params: {csvId?, dims, k?, features?, seed?, maxPoints?}`. `shapes`/`conns`/`scope:{nodeId}` são **opcionais e aditivos** — ausentes ⇒ base inteira, retrocompat total (byte-idêntico ao caminho pré-FR1, incl. seed). Com `scope`, o handler resolve `resolveScopeRowMask` e filtra as linhas antes da agregação (`model.scope` fica preenchido na resposta). O handler CLAMPA aos tetos declarados (`clampClusterParamsForBrowser`: dims ≤ 3, k ≤ 8, maxPoints 2000, sem extras) e roda `computeClusterSegments`; responde com `CLUSTER_SEGMENTS_RESULT` |
| `cluster_segments` | `{params}` (`params.scope?`/`params.rowMask?` quando escopado) | **Execução Híbrida H8 + Épico FR (DEC-FR-003, FR3)** — alias de FALLBACK da Clusterização (Classe B): mesma task do sidecar, params CLAMPADOS aos tetos browser. É o que o ComputeRouter posta quando o job do sidecar cai/está indisponível (paridade total, P4). Quando o job carregava `scope`/`rowMask` (job profundo escopado), o fallback decodifica a máscara e roda clampado sobre a MESMA subpopulação — nunca um cluster global silencioso quando o usuário pediu por nó; sem esses campos, caminho byte-idêntico ao de antes. Responde com o MESMO `CLUSTER_SEGMENTS_RESULT` |
| `COMPUTE_SCOPE_MASK` | `{shapes, conns, scope:{nodeId}}` | **Épico FR (DEC-FR-003, FR3)** — produz a máscara de linhas do escopo (via `resolveScopeRowMask`, o MESMO helper da FR1) para o job profundo do sidecar. O walk de política JAMAIS é portado ao Python — só a máscara viaja. Responde com `SCOPE_MASK_RESULT`. Chamado por `runDeepClusterSegments` (`src/App.jsx`) antes de montar `params.rowMask` do job `cluster_segments`; teto `FR_MASK_MAX_ROWS = 20_000_000` linhas — acima disso o escopo profundo cai declaradamente no worker clampado |
| `COMPUTE_RISK_BANDS` | `{csvId, col, metric, k?, autoK?, monotonic?, minShare?, shapes?, conns?, scope?}` | **Épico FR (DEC-FR-004/005/006/010, FR4)** — Criar Faixas por Risco: binning supervisionado por IV/WoE de uma coluna contínua (`computeRiskBands`). Classe A — **nunca roteia ao sidecar**, fora do cache do tick. `shapes`/`conns`/`scope:{nodeId,label}` opcionais (REUSA `resolveScopeRowMask` da FR1). Responde com `RISK_BANDS_RESULT` |
...
| `CLUSTER_SEGMENTS_RESULT` | `{clusterModel}` — ver `docs/wiki/Arquitetura-Execucao-Hibrida.md` (§7.3, §16) e `docs/claude/Copiloto-Clusterizacao.md`. `clusterModel.scope: null \| {nodeId, label}` é campo aditivo (Épico FR, FR1) |
| `SCOPE_MASK_RESULT` | `{csvId, rowCount, maskB64}` — bitmask little-endian base64 do escopo (Épico FR, FR3); ver `docs/claude/Copiloto-Clusterizacao.md` |
| `RISK_BANDS_RESULT` | `{rangeModel, reqTag}` — `RangeModel` (bandas com `label/min/max/qty/share/num/den/rate/woe`, `quality:{iv, ivUniform, monotonic, autoKReason}`, `error`); `reqTag` ecoado para a main descartar respostas obsoletas (mesmo padrão de `reqTokens` da Prévia AS IS). Ver `docs/claude/Copiloto-Clusterizacao.md` (Épico FR, FR4) |
