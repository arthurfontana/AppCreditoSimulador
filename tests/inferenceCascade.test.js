...
function buildControlCascade(headers, rows) {
  ...
  for (const r of rows) {
    ...
    const prem = { conv: parseFloat(r[iConv]) || 0, fpd: parseFloat(r[iFpd]) || 0, confiab: conf };
...
// Volume inferido (altas) por faixa de confiab da premissa usada — controle
// independente da agregação de confiabilidade do worker (Fase 3 / Proposta §4.5).
function controlConfiabVolume(normalizeScore) {
  const bi = {};
  baseHeaders.forEach((h, i) => { bi[h] = i; });
  const vol = { ALTA: 0, MEDIA: 0, BAIXA: 0, GLOBAL: 0 };
  for (const row of baseRows) {
    const peso = parseInt(row[bi[WEIGHT_COL]], 10) || 0;
    const sRaw = row[bi['SCORE_HVI3']];
    const s = normalizeScore ? normalizeScoreKey(sRaw) : String(sRaw ?? '').trim();
    ...
    const p =
      ...
      control.maps[1][s]                       ??
      control.global;
    const altas = peso * p.conv;
    if (altas > 0) {
      const lbl = (p.confiab || 'GLOBAL').toUpperCase();
      if (lbl in vol) vol[lbl] += altas; else vol.GLOBAL += altas;
    }
  }
  return vol;
}
...
describe('Confiabilidade propagada (Fase 3 / Proposta §4.5, CONTRATO §7)', () => {
  const shapes = [
    { id: 'lens', type: 'decision_lens', rules: [] },
    { id: 'appr', type: 'approved' },
  ];
  ...
  it('runSimulation em modo ref sinaliza a origem e devolve o breakdown de confiab', () => {
    const res = runSimulation(shapes, conns, { base: makeCsv(true) }, ref);
    expect(res.inferenceSource).toBe('ref');
    expect(res.confiabVolume).toBeTruthy();
    expect(res.confiabVolume).toHaveProperty('ALTA');

    const ctl = controlConfiabVolume(true);
    for (const k of ['ALTA', 'MEDIA', 'BAIXA', 'GLOBAL']) {
      expect(res.confiabVolume[k]).toBeCloseTo(ctl[k], 4);
    }
    // A soma das faixas = total de altas inferidas (mesma grandeza do volume inferido).
    const sum = Object.values(res.confiabVolume).reduce((a, b) => a + b, 0);
    const totalAltas = controlAggregate(true).altas;
    expect(sum).toBeCloseTo(totalAltas, 4);

    const pctAlta = res.confiabVolume.ALTA / sum;
    expect(pctAlta).toBeGreaterThanOrEqual(0);
    expect(pctAlta).toBeLessThanOrEqual(1);
    // eslint-disable-next-line no-console
    console.log(`[CONFIAB] %ALTA=${(pctAlta * 100).toFixed(2)} ` +
      Object.entries(res.confiabVolume).map(([k, v]) => `${k}=${v.toFixed(0)}`).join(' '));
  });

  it('modo columns não sinaliza origem nem confiab (retrocompatível)', () => {
    const csv = { ...makeCsv(true), inferenceConfig: { source: 'columns' } };
    const res = runSimulation(shapes, conns, { base: csv }, ref);
    expect(res.inferenceSource).toBeNull();
